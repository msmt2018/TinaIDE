...
$libsBase = (Resolve-Path (Join-Path $root 'external/embedded-ndk-libs')).Path
...
if (-not $running) {
  ...
  & pwsh (Join-Path $root 'docker/embedded-ndk/build-local.ps1') -Mode libs -Abi arm64-v8a | Out-Null
}
...
# 期望目录结构：/hostout-libs/common-headers/clang/Basic/Version.h
mkdir -p /hostout-libs/common-headers
cp -a /work/src/llvm-project/clang/include/. /hostout-libs/common-headers/
"@
...
# Verify on host
$hdr = Join-Path $libsBase 'common-headers/clang/Basic/Version.h'
if (Test-Path $hdr) {
  Info "OK: $hdr"
} else {
