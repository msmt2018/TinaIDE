...
.PARAMETER OutputPath
    Output directory (default: docker/proot-build/output).

.PARAMETER CopyToJniLibs
    Copy artifacts to app/src/prebuiltProot/jniLibs after a successful build.

.PARAMETER CopyToAssets
    Copy libtalloc.so.2 to app/src/<flavor>/assets/proot after a successful build.

.PARAMETER ResetSource
    Force re-clone source code (rarely needed).
...
.PARAMETER BuildSysroot
    Build multi-API sysroot packages instead of PRoot binaries.

.PARAMETER SysrootApiLevels
    Comma-separated list of API levels to build sysroot for (default: 21,24,28,29,30,31,33,34).
...
.EXAMPLE
    ...
    .\build-proot.ps1 -BuildSysroot             # build unified sysroot package (auto-detect API levels)
    .\build-proot.ps1 -BuildSysroot -SysrootApiLevels "28,29,30"  # build specific API levels only
#>
...
$SourceVolumeName = "tinaide-proot-source"
...
function Ensure-Buildx {
    if (-not (Test-BuildxMultiArch)) {
        ...
        docker buildx create --name tina-multiarch --use 2>$null | Out-Null
...
$ImageBaseName = "tinaide-proot-builder"
...
function Get-ContainerName($arch) {
    return "tinaide-proot-build-${arch}"
}
...
function Build-PRoot($arch, $outputDir) {
    ...
    # Ensure source volume exists (only for PRoot build)
    if (-not $BuildSysroot) {
        Ensure-SourceVolume
    }
    ...
    if ($BuildSysroot) {
        Write-Info "Building unified sysroot ($arch)..."
        if ($SysrootApiLevels) {
            Write-Info "  API Levels: $SysrootApiLevels (manual)"
        } else {
            Write-Info "  API Levels: auto-detect from NDK"
        }
    } else {
    ...
    if ($BuildSysroot) {
        # Sysroot build: no source volume needed
        $dockerArgs = @(
            'run',
            '--name', $containerName,
            '--entrypoint', '/build/scripts/build-sysroot.sh',
            '-v', "${scriptsPath}:/build/scripts:ro",
            '-v', "${outputDir}:/output",
            '-e', "TARGET_ARCH=$targetArch"
        )
        
        # 只在用户指定时才传递 API_LEVELS
        if ($SysrootApiLevels) {
            $dockerArgs += '-e'
            $dockerArgs += "API_LEVELS=$SysrootApiLevels"
        }
        
        $dockerArgs += $imageName
    } else {
        # PRoot build: use source volume
        $dockerArgs = @(
            'run',
            '--name', $containerName,
    ...
    # Verify outputs
    if ($BuildSysroot) {
        # Check for sysroot packages
        $sysrootFiles = Get-ChildItem -Path $outputDir -Filter "android-sysroot-*.tar.xz" -ErrorAction SilentlyContinue
        if ($sysrootFiles.Count -eq 0) {
            Write-Err "No sysroot packages found in output"
            return $false
        }
    } else {
        # Check for PRoot binaries
...
Write-Host ""
Write-Host "============================================" -ForegroundColor Cyan
Write-Host "  TinaIDE PRoot Builder" -ForegroundColor Cyan
Write-Host "============================================" -ForegroundColor Cyan
Write-Host ""
...
if ($BuildSysroot) {
    Write-Info "Build type: Unified Sysroot"
    if ($SysrootApiLevels) {
        Write-Info "API Levels: $SysrootApiLevels (manual)"
    } else {
        Write-Info "API Levels: auto-detect from NDK"
...
if ($allSuccess -and $CopyToJniLibs) {
    ...
    $jniLibsBase = Join-Path $projectRoot 'app\src\prebuiltProot\jniLibs'
...
if ($allSuccess -and $CopyToAssets) {
    ...
    foreach ($arch in $archList) {
        ...
        $targetDir = Join-Path $projectRoot "app\src\$flavorDir\assets\proot\$(switch ($arch) { 'arm64' { 'arm64-v8a' } 'x86_64' { 'x86_64' } })"
...
if ($allSuccess) {
    ...
    foreach ($arch in $archList) {
        ...
        if (Test-Path $dir) {
            ...
            if ($BuildSysroot) {
                Get-ChildItem $dir -Filter "*.tar.xz" -Recurse | ForEach-Object {
    ...
    if ($BuildSysroot) {
        Write-Host ""
        Write-Info "Next: Copy sysroot packages to app/src/<arch>/assets/android-sysroot/"
    } elseif (-not $CopyToJniLibs) {
        Write-Host ""
