...
- 开源版已经移除账号登录、QQ 登录、激活码/许可证、会员和官方 AI 额度入口；私有后端与管理端不再随公开仓库分发。
...
| 开源版身份策略 | 不内置账号登录、QQ 登录、激活码/许可证或会员入口 | `ProfileScreen.kt`, `AiConfigStrategy.kt` |
...
| 功能 | 说明 | 主要文件 |
|------|------|----------|
| 账号登录 | 开源版不提供账号系统入口 | `ProfileScreen.kt` |
| QQ 登录 | 开源版不集成 QQ SDK 与 OAuth 登录入口 | `app/build.gradle.kts`, `AndroidManifest.xml` |
