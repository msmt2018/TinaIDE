...
所有设计系统组件位于 `app/src/main/java/com/wuxianggujun/tinaide/ui/compose/components/` 目录（部分已迁移至 `core/designsystem/`）：
...
object TinaAnimation {
    // 动画时长常量
    const val Fast = 150     // 快速动画 - 状态切换、微交互
    const val Normal = 300   // 标准动画 - 页面切换、组件展开/收起
    const val Slow = 500     // 慢速动画 - 强调效果、引导动画

    // 缓动函数
    val StandardEasing = FastOutSlowInEasing  // 标准缓动，最常用
    val LinearEasing = LinearEasing           // 线性缓动，用于循环动画
    val EnterEasing = LinearOutSlowInEasing   // 进入缓动
    val ExitEasing = FastOutLinearInEasing    // 退出缓动

    // 预设 AnimationSpec
    fun <T> fastTween(easing: Easing = StandardEasing)    // 快速动画
    fun <T> normalTween(easing: Easing = StandardEasing)  // 标准动画
    fun <T> slowTween(easing: Easing = StandardEasing)    // 慢速动画
    fun <T> customTween(durationMillis: Int, ...)         // 自定义动画
}
```
...
| 时长 | 值 | 使用场景 |
|------|---|---------|
| `Fast` | 150ms | 状态切换、按钮反馈、微交互 |
| `Normal` | 300ms | 页面切换、组件展开/收起、淡入淡出 |
| `Slow` | 500ms | 强调效果、引导动画、首次加载 |
...
val backgroundColor by animateColorAsState(
    ...
    animationSpec = TinaAnimation.fastTween(),
...
| `TinaCenterAlignedTopBar` | 居中标题顶部栏 | 居中标题 |
| `TinaLargeTopBar` | 大标题顶部栏 | 可折叠大标题 |
| `TinaTopBarAction` | 顶部栏操作按钮 | 图标按钮 |
...
| `TinaOutlinedButtonLarge` | 大号轮廓按钮 | 轮廓样式，56dp 高度，全宽 |
...
| `TinaOutlinedCard` | 轮廓卡片 | 边框样式 |
| `TinaElevatedCard` | 高亮卡片 | 4dp 阴影 |
