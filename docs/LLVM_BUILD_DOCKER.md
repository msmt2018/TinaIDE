...
- 产物布局与 docs/EMBEDDED_NDK_TOOLS.md 一致。
...
# 产物直写 external/embedded-ndk-libs/<abi>/（含 libs 与 sysroot、MANIFEST、SHA256）
...
# 覆盖版本（如需）：
./docker/embedded-ndk/build-local.ps1 -Mode exec -Abi x86_64 -NdkVersion r26d -LlvmTag llvmorg-17.0.6
```

清理与重置
```
# 清理可执行工具链产物（arm64-v8a），并移除 assets/{toolchains,sysroot}
./docker/embedded-ndk/clean-local.ps1 -Mode exec -Abi arm64-v8a -RemoveAssets -Yes
# 清理所有 ABI + Docker 镜像
...
与 docs/EMBEDDED_NDK_TOOLS.md 的映射
- 最小集合：已按文档选取并打包。
