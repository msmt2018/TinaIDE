...
{
#ifdef TB_CONFIG_OS_ANDROID
    // TinaIDE: Android 上必须使用进程桥接，不允许回退到 fork/exec
    // 因为 Android 上没有独立的编译器可执行文件，必须通过 ProcessBridge 调用 NativeCompiler
    tb_process_ref_t process = tb_process_init_android(pathname, argv, attr);
    if (process) return process;
    
    // TinaIDE: 进程桥接失败，输出详细错误信息
    tb_trace_e("[TinaIDE] Android process bridge failed for command: %s", pathname ? pathname : "(null)");
    tb_trace_e("[TinaIDE] Possible causes:");
    tb_trace_e("[TinaIDE]   1. tb_android_init_env(jvm) was not called in JNI_OnLoad");
    tb_trace_e("[TinaIDE]   2. ProcessBridge class not found");
    tb_trace_e("[TinaIDE]   3. JNI environment not available");
    return tb_null;
#else
    // 非 Android 平台，使用标准实现
#ifdef TB_CONFIG_POSIX_HAVE_POSIX_SPAWNP
...
{
#ifdef TB_CONFIG_OS_ANDROID
    if (tb_process_is_android(self))
    {
        tb_process_exit_android(self);
        return;
    }
#endif
    // check
...
#ifdef TB_CONFIG_OS_ANDROID
    if (tb_process_is_android(self))
        return tb_process_priv_android(self);
#endif
    // check
    tb_process_t* process = (tb_process_t*)self;
...
tb_void_t tb_process_priv_set(tb_process_ref_t self, tb_cpointer_t priv)
{
    ...
    if (tb_process_is_android(self))
    {
        tb_process_priv_set_android(self, priv);
        return;
    }
#endif
    // check
...
tb_void_t tb_process_resume(tb_process_ref_t self)
{
    ...
    if (tb_process_is_android(self))
    {
        tb_process_resume_android(self);
        return;
    }
#endif
    // check
...
tb_void_t tb_process_suspend(tb_process_ref_t self)
{
    ...
    if (tb_process_is_android(self))
    {
        tb_process_suspend_android(self);
        return;
    }
#endif
    // check
...
#ifdef TB_CONFIG_OS_ANDROID
    if (tb_process_is_android(self))
        return tb_process_wait_android(self, pstatus, timeout);
#endif
    // check
    tb_process_t* process = (tb_process_t*)self;
    tb_assert_and_check_return_val(process, -1);
...
{
#ifdef TB_CONFIG_OS_ANDROID
    if (processes && processes[0] && tb_process_is_android(processes[0]))
        return tb_process_waitlist_android(processes, infolist, infomaxn, timeout);
#endif
    // check
