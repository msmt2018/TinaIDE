...
((argument) @type
  (#match? @type "\\c^(c|cxx|cuda|objc|objcxx|asm|asm_nasm|asm_masm)$"))
...
(normal_command
  ...
  (#match? @_function
    "\\c^(cmake_minimum_required|project|find_package|set_property|set_target_properties|write_basic_package_version_file)$")
  (argument_list
    (argument) @keyword.modifier
    (#match? @keyword.modifier "\\c^(version|soversion)$")))
...
(normal_command
  ...
  (argument_list
    (argument) @keyword.modifier
    (#match? @keyword.modifier "\\c^(version|languages|description|homepage_url)$")))

(normal_command
  (identifier) @_function
  (#match? @_function "\\c^find_package$")
  (argument_list
    (argument) @keyword.modifier
    (#match? @keyword.modifier
      "\\c^(config|module|required|quiet|exact|components|optional_components|names|hints|paths|path_suffixes|no_default_path)$")))

(normal_command
  (identifier) @_function
  (#match? @_function "\\c^add_library$")
  (argument_list
    (argument) @keyword.modifier
    (#match? @keyword.modifier
      "\\c^(static|shared|module|object|interface|unknown|alias|imported|global|exclude_from_all)$")))

(normal_command
  (identifier) @_function
  (#match? @_function "\\c^add_executable$")
  (argument_list
    (argument) @keyword.modifier
    (#match? @keyword.modifier
      "\\c^(win32|macosx_bundle|exclude_from_all|imported|alias)$")))

(normal_command
  (identifier) @_function
  (#match? @_function
    "\\c^(target_link_libraries|target_include_directories|target_link_directories|target_compile_definitions|target_compile_features|target_compile_options|target_link_options|target_sources)$")
  (argument_list
    (argument) @keyword.modifier
    (#match? @keyword.modifier "\\c^(private|public|interface|before|system)$")))

(normal_command
  (identifier) @_function
  (#match? @_function "\\c^set_target_properties$")
  (argument_list
    (argument) @keyword.modifier
    (#match? @keyword.modifier "\\c^(properties|output_name|version|soversion)$")))

(normal_command
  (identifier) @_function
  (#match? @_function "\\c^install$")
  (argument_list
    (argument) @keyword.modifier
    (#match? @keyword.modifier
      "\\c^(targets|files|directory|export|destination|component|configurations|permissions|runtime|library|archive|framework|bundle|includes|optional|rename|pattern|regex)$")))

(normal_command
  (identifier) @_function
  (#match? @_function "\\c^message$")
  (argument_list
    (argument) @keyword.modifier
    (#match? @keyword.modifier
      "\\c^(fatal_error|send_error|warning|notice|status|verbose|debug|trace|check_start|check_pass|check_fail|deprecation|author_warning)$")))

(normal_command
  (identifier) @_function
  (#match? @_function "\\c^(add_custom_command|add_custom_target|execute_process)$")
  (argument_list
    (argument) @keyword.modifier
    (#match? @keyword.modifier
      "\\c^(output|outputs|command|depends|byproducts|comment|working_directory|verbatim|uses_terminal|command_expand_lists|append|main_dependency|depfile|result_variable|output_variable|error_variable|timeout)$")))
